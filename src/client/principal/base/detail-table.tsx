...
import type {FixedFields, TableDefinition} from 'frontend-plus';
...
import {EstadoRegistro, Fila, mensajeDeError} from './tipos-tabla';
...
export function DetailTable({
    ...
    const columnas = React.useMemo<GridColDef[]>(() => {
        ...
        return definicion.fields
            .filter(field => !ocultas.has(field.name) && field.inTable !== false && !field.clientSide)
            .map(field => ({
                field:field.name,
...
function PanelDeRegistro({
    ...
    const camposVisibles = React.useMemo(
        () => definicion.fields.filter(field =>
            nombresFijos.indexOf(field.name) < 0 && !field.clientSide
        ),
    ...
    const borrar = React.useCallback(async () => {
        ...
        try{
            await conn.ajax.table_record_save({
                ...
                oldRow:filaInicial as never,
                status:'delete' as EstadoRegistro as never,
            });
            onGuardado();
        }catch(err){
